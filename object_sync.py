...
def parse_args():
    ...
    parser.add_argument('-m', '--method', metavar='METHOD', dest='method', type=str, default='',
                       help='Method used to compute the model.')
...
def househould_insert(thumbnail, meshfile, method, obj):
    call = ['rosrun', 'object_recognition_household_object_database_sync', 'insert_model', '-G', meshfile, '-T', thumbnail, '-N', obj.object_name, '-D', obj.description, '-I', str(obj.id), '-M', method]
    call += list(obj.tags)
    print 'Inserting:\n', ' '.join(call)
